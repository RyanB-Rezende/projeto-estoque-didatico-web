...
export const getUsuarios = async () => {
  try {
    const { data, error } = await supabase
      .from('usuarios')
      .select('id_usuario, nome, telefone, email, endereco, cargo, senha, status, turma, cpf, data_nascimento, cargos(nome), turmas(nome)')
...
export const getUsuarioById = async (id) => {
  try {
    ...
    const { data, error } = await supabase
      .from('usuarios')
      .select('*')
      .eq('id_usuario', isNumericId ? Number(id) : String(id))
      .single();
...
export const updateUsuario = async (id, usuarioData) => {
  try {
    ...
    if (error) {
      ...
      throw new Error('Erro no serviço de atualização');
    }
